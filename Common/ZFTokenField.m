...
@property (nonatomic ,strong) UIImageView *textFieldButtomView;
@end
...
-(UIImageView *)textFieldButtomView
{
    if ( _textFieldButtomView == nil ) {
        _textFieldButtomView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH - 30, 5)];
        _textFieldButtomView.image = [UIImage imageNamed:@"textField_buttom_line.png"];
    }
    return _textFieldButtomView;
}
...
- (void)reloadData
{
    ...
    self.textField.frame = (CGRect) {0,0,80,[self.dataSource lineHeightForTokenInField:self]};
    [self addSubview:self.textFieldButtomView];
...
- (void)enumerateItemRectsUsingBlock:(void (^)(CGRect itemRect))block
{
    ...
    for (UIView *token in self.tokenViews) {
        ...
        if ([token isKindOfClass:[ZFTokenTextField class]]) {
            ...
            self.textFieldButtomView.frame = (CGRect){0,token.bottom,self.textFieldButtomView.width,self.textFieldButtomView.height};
            
            if ( [self.delegate respondsToSelector:@selector(textFieldViewOffsetY:)] ) {
                [self.delegate textFieldViewOffsetY:self.textFieldButtomView.bottom];
            }
        }
...
- (BOOL)textField:(ZFTokenTextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string
{
    [self enumerateItemRectsUsingBlock:^(CGRect itemRect) {
    }];
    
    if ([self.delegate respondsToSelector:@selector(tokenField:shouldChangeCharactersInRange:replacementString:)]) {
        return [self.delegate tokenField:textField shouldChangeCharactersInRange:range replacementString:string];
    }
    else{
        return YES;
    }
